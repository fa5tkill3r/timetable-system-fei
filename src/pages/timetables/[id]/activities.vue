<script setup lang="ts">
  ...
  const formatLecturerNames = (subjectId: number) => {
    ...
    return lecturers
      .map(
        (lecturer) =>
          (lecturer.user as any as User)?.full_name ||
          (lecturer.user as any as User)?.username ||
          'Unknown',
      )
      .join(', ')
  }
  ...
  const filteredEvents = computed(() => {
    ...
    if (selectedRoomIds.value.length > 0) {
      result = result.filter(
        (event) => event.room && selectedRoomIds.value.includes(event.room),
      )
    }
    ...
    if (selectedSubjectIds.value.length > 0) {
      result = result.filter(
        (event) =>
          (event.tta as any)?.subject &&
          selectedSubjectIds.value.includes((event.tta as any).subject),
      )
    }
    ...
    if (selectedLecturerIds.value.length > 0) {
      result = result.filter((event) => {
        ...
        return lecturers.some(
          (lecturer) =>
            (lecturer.user as any as User)?.id &&
            selectedLecturerIds.value.includes(
              (lecturer.user as any as User).id!,
            ),
        )
      })
    ...
    if (selectedGroups.value.length > 0) {
      result = result.filter((event) => {
        ...
        return groups.some(
          (group) => group.name && selectedGroups.value.includes(group.name),
        )
      })
    }
    ...
    if (selectedTypes.value.length > 0) {
      result = result.filter(
        (event) =>
          (event.tta as any)?.event_type &&
          selectedTypes.value.includes((event.tta as any).event_type),
      )
    }
    ...
    if (selectedStatuses.value.length > 0) {
      result = result.filter((event) => {
        ...
        if (selectedStatuses.value.includes('unplaced') && !isPlaced)
          return true
        return false
      })
    }
  ...
  const columns: ColumnDef<TTEvent>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const day =
          daysOfWeek.find((d) => d.value === dayCode)?.label || dayCode
        return day
      },
    },
    // {
  ...
  async function handleDelete() {
    ...
    try {
      const success = await timetableEventStore.deleteEvent(
        itemToDelete.value,
        timetableId,
      )
...
<template>
  <div class="flex flex-1 flex-col space-y-4 px-8 pt-6">
      ...
      <div class="flex flex-col space-y-2">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbLink :href="`/timetables/${timetableId}`"
                >{{ currentTimetable?.name || 'Timetable' }}
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
    ...
    <!-- Filters -->
    <div class="space-y-2 rounded-md border bg-muted/40 p-2">
      <!-- Search input and first row of filters -->
      <div class="flex items-center space-x-2">
        <div class="relative">
          <Input
            placeholder="Search by subject, code or lecturer..."
            v-model="searchTerm"
            class="h-8 w-[300px] pr-16"
          />
          <div
            v-if="searchTerm.trim()"
            class="absolute right-2 top-1/2 -translate-y-1/2 transform text-xs text-muted-foreground"
          >
