...
async function saveRoom(roomData: any) {
  ...
  try {
    if (selectedRoom.value?.id) {
      // Update
      const result = await buildingStore.updateRoom(selectedRoom.value.id, roomData)
      if (result) {
        toast({
          title: "Room updated",
...
async function handleDelete() {
  ...
  dialogLoading.value = true
  try {
    const success = await buildingStore.deleteRoom(itemToDelete.value, buildingId.value)
