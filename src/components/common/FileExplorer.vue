<template>
  <div class="space-y-4">
    <div class="flex items-center space-x-2">
      ...
      <div class="flex-1 overflow-x-auto rounded-md bg-muted px-3 py-1 text-sm">
    ...
    <div class="h-[400px] overflow-y-auto rounded-md border">
      ...
      >
        <div
          ...
          class="flex cursor-pointer items-center p-3 transition-colors hover:bg-accent"
          :class="{
            'bg-primary/10': selectedPath === getCurrentFullPath(item),
          }"
        >
          <FolderIcon
          ...
          <FileIcon
            v-else
            class="mr-2 h-4 w-4"
            :class="isCsvFile(item.name) ? 'text-green-500' : 'text-gray-500'"
          />
          <span>{{ item.name }}</span>
          <span
            v-if="selectedPath === getCurrentFullPath(item)"
            class="ml-2 rounded-full bg-primary/20 px-2 py-0.5 text-xs"
            >Selected</span
          >
        </div>
        <div
          v-if="directoryContents.length === 0"
    ...
    <div class="rounded-md bg-muted p-3">
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          <FolderIcon class="mr-2 h-4 w-4 text-primary" />
          <span
            >Current directory:
            <span class="font-medium">{{ currentPath || '/' }}</span></span
          >
        </div>
        <div class="flex items-center">
          <span
...
<script setup lang="ts">
  ...
  const csvFileCount = computed(() => {
    return directoryContents.value.filter(
      (item) => !item.isDirectory && isCsvFile(item.name),
    ).length
  })
  ...
  async function fetchDirectoryContents() {
    ...
    try {
      ...
      const directories = ((responseData?.dirs as string[]) || []).map(
        (name: string) => ({
          name,
          isDirectory: true,
        }),
      ...
      const files = ((responseData?.files as string[]) || []).map(
        (name: string) => ({
          name,
          isDirectory: false,
        }),
