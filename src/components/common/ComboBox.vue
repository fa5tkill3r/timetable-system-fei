<script setup lang="ts">
  ...
  const displayText = computed(() => {
    ...
    if (props.multiple && Array.isArray(selection.value)) {
      ...
      const first = props.options.find(
        (opt) => opt.id === selection.value[0],
      )?.name
    ...
    return (
      props.options.find((option) => option.id === selection.value)?.name ||
  ...
  >
    <FormItem class="flex flex-col">
      ...
      <Popover v-model:open="isOpen">
            ...
            <Button
              ...
              :class="
                cn(
                  ...
                  !values.selection && 'text-muted-foreground',
                )
              "
            >
        ...
        <PopoverContent class="w-fit min-w-[200px] max-w-[400px] p-0">
          <Command>
            <CommandInput :placeholder="searchPlaceholder" />
            <!-- Selected tags/pills for multi-select -->
            <div
              v-if="
                multiple && Array.isArray(selection) && selection.length > 0
              "
            ...
            >
              ...
              >
                ...
                <button
                  @click.stop="removeItem(selectedId)"
                  type="button"
                  class="flex h-4 w-4 items-center justify-center rounded-full hover:bg-muted"
                >
                  <X class="h-3 w-3" />
                </button>
              </div>
