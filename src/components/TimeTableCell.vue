<script setup lang="ts">
  ...
  const colSpan = computed(() =>
    items.value.reduce((acc, item) => acc + item.duration, 0),
  )
...
<template>
  <td class="w-32" :colspan="colSpan">
    <div
      ref="target"
      class="border w-full h-16 flex flex-col justify-center items-center"
    >
      <div
        v-for="item in items"
        :key="item"
        class="text-black w-full h-full text-center table"
