<template>
  ...
  >
    <DialogContent class="sm:max-w-[425px]">
      <DialogHeader>
        <DialogTitle>{{
          roomEquipment ? 'Edit Room Equipment' : 'Add Equipment to Room'
        }}</DialogTitle>
        <DialogDescription>
          {{
            roomEquipment
              ? 'Update room equipment details.'
      ...
      <form @submit.prevent="handleSubmit">
        <div class="grid gap-4 py-4">
          ...
          <div class="grid grid-cols-4 items-center gap-4">
            ...
            <div
              class="col-span-3 rounded-md border bg-muted/50 px-3 py-2 text-sm"
            >
              {{ roomName }}
            </div>
          </div>
