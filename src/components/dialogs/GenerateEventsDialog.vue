  ...
  const formSchema = toTypedSchema(
    z.object({
      subjectGroupName: z
        .string()
        .array()
        .min(1, 'At least one subject group is required'),
    }),
  )
    ...
    try {
      ...
      for (const groupName of groups) {
        ...
        try {
          ...
          results.failed++
          results.errors.push(
            `${groupName}: ${error instanceof Error ? error.message : 'Unknown error'}`,
          )
        }

        processedGroups.value++
        progressPercentage.value = Math.round(
          (processedGroups.value / totalGroups.value) * 100,
        )
      }
    ...
    <DialogContent class="sm:max-w-[500px]">
      <DialogHeader>
        ...
        <DialogDescription>
          ...
          {{ timetable?.name || 'this timetable' }} using the options below.
        </DialogDescription>
      </DialogHeader>
      ...
      >
        ...
        <div
          v-if="isGenerating && totalGroups > 1"
        ...
        <DialogFooter>
          ...
          >
            <Loader2
              v-if="isGenerating || props.isLoading"
