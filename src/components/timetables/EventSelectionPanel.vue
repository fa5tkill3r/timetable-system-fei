...
<template>
  ...
  <div ref="containerRef" class="relative h-full bg-white p-4" :class="{ 'bg-gray-50': isOverMenu }"
    @dragover="handleMenuDragOver" @dragleave="handleMenuDragLeave" @drop="handleMenuDrop">
    <Tabs default-value="events" class="h-full">
      <!-- Hide tabs when in compact mode -->
      <TabsList v-if="!isCompactMode" class="w-full">
        <TabsTrigger value="events" class="w-full">{{ $t('timetable.editor.eventSelection.title') }}</TabsTrigger>
        <TabsTrigger value="requirements" class="w-full">{{ $t('timetable.editor.eventSelection.requirements') }}
        </TabsTrigger>
      </TabsList>

      <TabsContent value="events" class="h-[calc(100%-40px)] overflow-y-auto" :class="{ 'h-full': isCompactMode }">
        <div v-if="!isCompactMode" class="mb-4 flex items-center justify-between">
          <h3 class="text-lg font-semibold">{{ $t('timetable.editor.eventSelection.title') }}</h3>
          <Badge>{{ filteredEventTemplates.length }}</Badge>
        </div>

        <div v-if="!isCompactMode" class="mb-4 space-y-2">
          ...
          <div class="flex flex-wrap gap-2">
            <ComboBoxFilter :title="$t('timetable.editor.eventSelection.type')" :options="eventTypeOptions"
              v-model="selectedEventTypeIds" class="h-8" />

            <ComboBoxFilter :title="$t('timetable.editor.eventSelection.group')" :options="groupOptions"
              v-model="selectedGroupIds" class="h-8" />

            <Button v-if="isFiltered" variant="ghost" size="sm" class="h-8" @click="resetFilters">
              {{ $t('reset') }}
        ...
        <div class="space-y-3">
          <div v-for="template in filteredEventTemplates" :key="`${template.title}_${template.event_type}`"
            v-show="(template.quantity || 0) > 0" class="group relative cursor-move rounded-lg p-2"
            :class="{ 'p-3': !isCompactMode }" :style="{ backgroundColor: getAdjustedColor(template) }" draggable="true"
            @dragstart="handleDragStart($event, template)" @dragend="handleDragEnd">
            ...
            <div class="space-y-0.5 text-gray-600" :class="{ 'text-xs': isCompactMode, 'text-sm': !isCompactMode }">
              ...
              <div class="truncate">{{ $t('timetable.editor.eventSelection.duration') }}: {{ template.duration || 1 }}h
              </div>
              <div class="truncate">{{ $t('timetable.editor.eventSelection.remaining') }}: {{ template.quantity || 0 }}
              </div>
            </div>
          </div>
        </div>

        <div v-if="filteredEventTemplates.length === 0" class="py-8 text-center text-muted-foreground">
        ...
        <div v-if="isOverMenu && isLoading === false"
          ...
          <div class="flex items-center text-blue-600">
            ...
            <span>{{ $t('timetable.editor.eventSelection.dropToRemove') }}</span>
          </div>
        </div>
      </TabsContent>
      ...
      <TabsContent value="requirements">
        <div class="space-y-4 pt-2">
          <h3 class="text-lg font-semibold">{{ $t('timetable.editor.eventSelection.requirements') }}</h3>
