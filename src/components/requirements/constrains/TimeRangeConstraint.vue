...
import { ref, CSSProperties, onMounted, onUnmounted, watch, nextTick } from 'vue'
...
function updateConstraintsFromCells() {
  try {
        ...
        if (bestMatch) {
          ...
          if (bestMatch.action === 'keep') {
            ...
            if (segment.start > constraintStart) {
              newConstraints.push({
                ...matchedOriginalConstraint,
                id: !originalIdUsedForThisGroup && matchedOriginalConstraint.id != null ? matchedOriginalConstraint.id : undefined,
                parent: undefined, nested_children: undefined,
    ...
    toast({
      ...
      description: t('constraints.timeRange.errorUpdating') || 'Failed to update constraints',
      variant: 'destructive'
    });
  }
}
...
<template>
  <div class="flex flex-col mx-auto p-6" @mouseup="handleMouseUp" @mouseleave="handleMouseUp">
    ...
    <div class="mb-6">
      ...
      <div class="flex gap-4 mb-4">
        <Button v-for="level in availabilityLevels" :key="level.id"
          :variant="selectedLevel?.id === level.id ? 'default' : 'outline'" class="flex items-center gap-2" :style="{
    ...
    <div class="h-[calc(100vh-300px)] select-none relative">
      ...
      <div v-if="isUpdating" class="absolute top-0 left-0 bg-white/60 flex items-center justify-center z-10" :style="{
        ...
        <div class="flex flex-col items-center">
          ...
          <span class="mt-2 text-sm font-medium text-gray-700">{{ $t('saving') }}</span>
        </div>
      </div>
    </div>
  </div>
